class TopTen < ActiveRecord::Base
	...
	def self.get_all_from_issue(issue_id)
		TopTen.where(issue_id: issue_id)
	end

	# Static method that gets a random top ten from the specified issue
	def self.get_random_from_issue(issue_id)
		all_from_issue = TopTen.get_all_from_issue(issue_id)
